...
public class CommonUtil {
    ...
    /**
     * 민감한 필드/헤더인지 확인 (키워드 기반)
     * @param name 필드명 또는 헤더명
     * @param keywords 마스킹 키워드 목록
     * @return 민감 여부
     */
    public static boolean isSensitive(String name, List<String> keywords) {
        if (name == null || keywords == null || keywords.isEmpty()) {
            return false;
        }
    ...
    /**
     * 마스킹 값 반환 (null이면 기본값)
     * @param masking 마스킹 설정
     * @return 마스킹 값
     */
    public static String getMaskValue(SuhLoggerProperties.MaskingConfig masking) {
        if (masking == null || masking.getMaskValue() == null) {
            return DEFAULT_MASK_VALUE;
        }
        return masking.getMaskValue();
    }

    /**
     * 파라미터 맵에 마스킹 적용
     * @param params 원본 파라미터 맵
     * @param maskFields 마스킹할 필드 키워드 목록
     * @param maskValue 마스킹 값
     * @return 마스킹 적용된 파라미터 맵
     */
    public static Map<String, Object> maskParameters(Map<String, Object> params, List<String> maskFields, String maskValue) {
        ...
        String mask = (maskValue != null) ? maskValue : DEFAULT_MASK_VALUE;
        Map<String, Object> masked = new HashMap<>();

        for (Map.Entry<String, Object> entry : params.entrySet()) {
            String paramName = entry.getKey();
            if (isSensitive(paramName, maskFields)) {
                masked.put(paramName, mask);
            } else {
                masked.put(paramName, entry.getValue());
            }
        }
     ...
     * @param maskHeaders 마스킹할 헤더 키워드 목록
     * @param maskValue 마스킹 값
     * @return 마스킹 적용된 헤더 맵
     */
    public static Map<String, String> maskHeaders(Map<String, String> headers, List<String> maskHeaders, String maskValue) {
        ...
        for (Map.Entry<String, String> entry : headers.entrySet()) {
            ...
            if (isSensitive(headerName, maskHeaders)) {
                masked.put(headerName, mask);
            } else {
                masked.put(headerName, entry.getValue());
            }
        }
    ...
    /**
     * @deprecated isSensitive 사용 권장
     */
    @Deprecated
    public static boolean isSensitiveField(String fieldName, List<String> maskFields) {
        return isSensitive(fieldName, maskFields);
    }
    ...
    public static Map<String, Object> createSafeMap(Object obj) {
        return createSafeMap(obj, null);
    }

    /**
     * 객체를 안전하게 Map으로 변환 (마스킹 설정 포함)
     * @param obj 변환할 객체
     * @param masking 마스킹 설정 (null이면 마스킹 안함)
     */
    public static Map<String, Object> createSafeMap(Object obj, SuhLoggerProperties.MaskingConfig masking) {
        if (obj == null) {
            return new HashMap<>();
        }
        ...
        // 마스킹 설정 확인
        boolean shouldMask = masking != null && masking.isEnabled();
        List<String> maskFields = shouldMask ? masking.getMaskFields() : null;
        String maskValue = (masking != null && masking.getMaskValue() != null)
            ? masking.getMaskValue() : "****";
        ...
        try {
            for (Field field : obj.getClass().getDeclaredFields()) {
                ...
                // 민감한 필드인 경우 마스킹 처리
                if (shouldMask && isSensitiveField(fieldName, maskFields)) {
                    result.put(fieldName, maskValue);
                    continue;
                }
