...
**개선**
- 로그가 이제 프로젝트의 로그 설정을 그대로 따릅니다
- 로그 색상·형식·레벨을 프로젝트에서 자유롭게 지정할 수 있습니다
- 로그를 파일이나 외부 수집기로 함께 내보낼 수 있습니다
- 기존 코드 수정 없이 버전만 올리면 바로 적용됩니다

**기타**
- 배포 및 버전 관리 자동화가 개선되었습니다
...
**New Features**
- 메서드 로깅을 위한 새로운 어노테이션 추가 (@LogCall, @LogMonitor)
- TriState 기반 헤더 로깅 제어 기능
- 민감한 데이터 마스킹 설정 추가

**Documentation**
- 빠른 시작 가이드 및 어노테이션 설명서 추가
- 설정, 마스킹, 헤더 로깅 관련 상세 문서 추가
- API 참고 문서 및 문제 해결 가이드 추가
...
**새로운 기능**
- 자동 QA 이슈 생성 기능 추가
- 변경 로그 자동 생성 및 관리 기능 개선
- 깃허브 라벨 자동 동기화 기능 추가

**개선 사항**
- 이슈 템플릿 및 PR 템플릿 업데이트
- 버전 관리 및 배포 워크플로우 개선
- 개발자 도구 및 유틸리티 추가

**Chores**
- 프로젝트 버전 업그레이드 (1.3.11 → 1.4.1)
- 저장소 설정 및 구성 파일 업데이트
...
**스타일**
- 로그 출력 가독성 개선: 타임스탬프 뒤 불필요한 공백 제거, 로그 레벨을 가변 간격으로 정렬해 정렬 품질 향상.

**문서**
- CHANGELOG에 1.3.9 항목 추가 및 최신 버전/날짜 갱신.
- README 버전 표기 업데이트.

**작업**
- 버전 업데이트: 1.3.9 → 1.3.11.
- 자동 변경 로그 워크플로우 개선: 요약 요청 토큰 교체 및 PR 제목 형식 안내 댓글 자동 게시.
- 메타데이터(lastUpdated 등) 갱신.
...
**신기능/변경 사항**
- 로그 출력 가독성 향상: ISO 8601 타임스탬프(타임존 포함), 로그 레벨/클래스명/프로세스 ID 색상 표시, 레벨 고정폭 정렬 및 필드 너비 확장 적용.

**문서**
- README 및 변경 로그의 버전 표기 최신화.

**작업(Chores)**
- 빌드 및 메타데이터 버전 업데이트(예: 1.3.8→1.3.9), 신규 릴리스 항목 추가(1.3.8).
...
**신기능/변경 사항**
- 로그 출력 포맷 업데이트: 색상 제거, 타임스탬프에서 시간대 표시 제거, 로그 레벨 고정폭 정렬, 스레드/클래스명 필드 폭 확대, 예외 및 스택 트레이스에서 색상 제거로 가독성 일관성 향상.

**문서**
- README 버전 표기 v1.3.6 → v1.3.7로 업데이트.
- 버전 관리 문서에서 버전 1.3.7 → 1.3.8 및 메타데이터 최신화.

**작업(Chores)**
- 빌드 아티팩트 버전 1.3.8로 상승.
